...
function renderSeatGrid() {
    ...
    // Calculate max seats per row to determine scaling
    let maxSeatsInRow = 0;
    ...
    // Apply classes based on maxSeatsInRow
    $grid.removeClass('seat-grid-small seat-grid-smaller'); // Remove previous classes
    if (maxSeatsInRow >= 15) {
        $grid.addClass('seat-grid-smaller'); // For 15+ seats, 1/3rd size
    } else if (maxSeatsInRow >= 10) {
        $grid.addClass('seat-grid-small'); // For 10-14 seats, 1/2 size
    }
